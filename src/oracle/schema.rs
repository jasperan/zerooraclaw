...
CREATE TABLE ZERO_MEMORIES (
    ...
    namespace       VARCHAR2(128)   DEFAULT 'default' NOT NULL,
...
const INDEXES: &[&str] = &[
    ...
    "CREATE INDEX idx_zero_memories_namespace ON ZERO_MEMORIES(namespace)",
    "CREATE INDEX idx_zero_memories_superseded ON ZERO_MEMORIES(superseded_by)",
...
fn migrate_zero_memories_schema(conn: &Connection) -> anyhow::Result<()> {
    const MEMORY_ALTERS: &[&str] = &[
        "ALTER TABLE ZERO_MEMORIES ADD namespace VARCHAR2(128) DEFAULT 'default' NOT NULL",
        "ALTER TABLE ZERO_MEMORIES ADD importance NUMBER(5,2) DEFAULT 0.5",
        "ALTER TABLE ZERO_MEMORIES ADD access_count NUMBER(10) DEFAULT 0",
        "ALTER TABLE ZERO_MEMORIES ADD superseded_by VARCHAR2(64)",
    ];

    for ddl in MEMORY_ALTERS {
        exec_ddl_idempotent(conn, ddl, &[ORA_COLUMN_ALREADY_EXISTS])?;
    }
...
pub fn init_schema(conn: &Connection, agent_id: &str) -> anyhow::Result<()> {
    ...
    migrate_zero_memories_schema(conn)?;
...
mod tests {
    ...
    fn index_count_is_correct() {
        // 6 on MEMORIES + 1 on DAILY_NOTES + 1 SESSIONS + 2 TRANSCRIPTS
        // + 1 STATE + 1 CONFIG + 1 PROMPTS = 13
