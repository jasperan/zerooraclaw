...
TOMLEOF
  ...
  if [ -n "$ADB_WALLET_BASE64" ]; then
    WALLET_DIR="/home/opc/.zeroclaw/wallet"
    mkdir -p "$WALLET_DIR"
...
sudo -u opc bash -c 'export ORACLE_HOME=/usr/lib/oracle/23/client64 && export LD_LIBRARY_PATH=$ORACLE_HOME/lib && /usr/local/bin/zeroclaw setup-oracle'

# -- 9. Run onboard --
echo "--- Running onboard ---"
sudo -u opc bash -c 'export ORACLE_HOME=/usr/lib/oracle/23/client64 && export LD_LIBRARY_PATH=$ORACLE_HOME/lib && /usr/local/bin/zeroclaw onboard --memory oracle' <<< "n" || true
