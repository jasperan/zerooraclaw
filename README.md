...
>
> ```bash
> curl -fsSL https://raw.githubusercontent.com/jasperan/zerooraclaw/main/install.sh | bash
> ```
>
...
> ```bash
> PROJECT_DIR=/opt/myapp curl -fsSL https://raw.githubusercontent.com/jasperan/zerooraclaw/main/install.sh | bash
> ```
>
> Or install manually:
> ```bash
...
```bash
./target/release/zeroclaw setup-oracle
./target/release/zeroclaw onboard --memory oracle
```

### 4. Chat

```bash
./target/release/zeroclaw agent -m "Hello! Remember that I love Rust."
./target/release/zeroclaw agent -m "What programming language do I like?"
```

### 5. Inspect

```bash
./target/release/zeroclaw oracle-inspect
./target/release/zeroclaw oracle-inspect memories --search "programming"
```

## Docker Compose

```bash
# Local Oracle AI Database 26ai Free for development
docker compose up oracle-db -d

# Create or refresh the zerooraclaw database user/grants
./scripts/setup-oracle.sh
```

The Oracle AI Database 26ai Free container takes approximately 2 minutes to initialize on first start.
...
5. **Configure ZeroOraClaw** (`~/.zeroclaw/config.toml`):
   ```toml
   [provider]
   name = "openai"
   ...
   ```bash
   PROVIDER=openai API_KEY=oci-genai ./target/release/zeroclaw agent -m "Hello"
   ```
...
Use `zeroclaw config schema` to inspect the full current config surface.
...
# Start chatting
ssh opc@<instance-ip> -t zeroclaw agent
